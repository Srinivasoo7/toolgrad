...
class ToolGradState(states.ApiUseState):
  ...
  kg_context: str = ""  # Optional domain knowledge grounding inverse prediction.
...
def sample_apis_or_end(
  ...
  Args:
    api_sampler: Optional callable ``(tools, num_apis) -> tools`` overriding
      the uniform random sampling in ``mcp.get_mcp_apis`` (MCP path only).
  """
  # Initialization
...
def inverse_predictor(toolgrad_state: ToolGradState,) -> ToolGradState:
  ...
  workflow_updater = module_lib.create_workflow_updater(
      kg_context=toolgrad_state.kg_context or "")
